...
module lab1( input logic        CLOCK_50,  // 50 MHz Clock input
   ...
   // ---- Both-pressed suppression ----
   // If both KEY[0] and KEY[1] are held at the same time, suppress all
   // actions until both are fully released.
   logic both_block = 1'b0;
   always_ff @(posedge clk) begin
      if (~key_clean[0] & ~key_clean[1])    // both pressed
         both_block <= 1'b1;
      else if (key_clean[0] & key_clean[1]) // both released
         both_block <= 1'b0;
   end

   // ---- Increment / decrement actions (KEY[0] has priority) ----
   logic inc, dec;
   assign inc = ~both_block & (key_fell[0] | (~key_clean[0] & rep_tick));
   assign dec = ~both_block & (key_fell[1] | (~key_clean[1] & rep_tick)) & ~inc;

...
module lab1( input logic        CLOCK_50,  // 50 MHz Clock input
   ...
   // ---- Debounce: require stable ~1ms before accepting change ----
   localparam [15:0] DB_THRESH = 16'd49_999;   // 1 ms at 50 MHz
   logic [3:0]  key_db = 4'b1111;              // debounced (init: all released)
   logic [15:0] db0 = 16'd0, db1 = 16'd0, db2 = 16'd0, db3 = 16'd0;

   always_ff @(posedge clk) begin
      // KEY[0]
      if (key_s2[0] == key_db[0])                                    db0 <= 16'd0;
      else if (db0 == DB_THRESH) begin db0 <= 16'd0; key_db[0] <= key_s2[0]; end
      else                                                           db0 <= db0 + 16'd1;
      // KEY[1]
      if (key_s2[1] == key_db[1])                                    db1 <= 16'd0;
      else if (db1 == DB_THRESH) begin db1 <= 16'd0; key_db[1] <= key_s2[1]; end
      else                                                           db1 <= db1 + 16'd1;
      // KEY[2]
      if (key_s2[2] == key_db[2])                                    db2 <= 16'd0;
      else if (db2 == DB_THRESH) begin db2 <= 16'd0; key_db[2] <= key_s2[2]; end
      else                                                           db2 <= db2 + 16'd1;
      // KEY[3]
      if (key_s2[3] == key_db[3])                                    db3 <= 16'd0;
      else if (db3 == DB_THRESH) begin db3 <= 16'd0; key_db[3] <= key_s2[3]; end
      else                                                           db3 <= db3 + 16'd1;
   end
   ...
   always_ff @(posedge clk) begin
      if (key_db[0] & key_db[1]) begin        // neither button held
         rep_cnt      <= 25'd0;
         initial_wait <= 1'b1;
